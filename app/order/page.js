...
export default function Orders() {
    ...
    useEffect(() => {
        async function fetchDocuments() {
            try{
            const imageResponse = await homeRaw("orderPg");
    ...
    return (
      ...
      { dataImg.map((doc)=>(
         <div key={doc._id} className="border border-gray-500  hover:bg-gray-700 rounded-lg p-1 md:p-3 w-80 md:w-auto flex flex-row"
         >
             <Image className="md:object-cover rounded-lg w-32 h-40"  width={100} height={100} alt="food" src={doc.image}/>
             <div className="m-2"> 
                 <h1 className=" text-xl md:text-3xl font-semibold" >{doc.name}</h1>
                 <p className="text-lg text-gray-500 mt-2">{doc.disc}</p>
